...
* **Pendiente:** aplicar la migración (usuario); prueba en producción real; Bloque 2 (extensión sube el XLSX a `yastas_movimientos_portal` y borra el archivo con `chrome.downloads.removeFile`, botón "Descargar del portal" en la ventana); Bloque 3 (botones ⚡ de corrección de errores de captura, siempre vía `AJUSTE_DE_SALDO`). `INSTRUCCIONES_CLAUDE_ENCUADRE.md` §"Decisiones de diseño" y el `encuadre_mockup.html` de la raíz quedaron desactualizados frente a estas reglas; la fuente de verdad es `PLAN_YASTAS_ENCUADRE.md` + `yastas-extension/docs/`.
