...
2. **RIGOR FINANCIERO, FUENTE ÚNICA DE VERDAD Y DOBLE AFECTACIÓN:**
   - La precisión numérica es CRÍTICA. Evita errores de redondeo trabajando siempre a 2 decimales exactos.
   - **PROHIBIDO LOCALSTORAGE PARA ESTADOS DE TURNO:** El estado de la sesión (`ABIERTO`/`CERRADO`) y saldos de caja NUNCA deben depender de `localStorage`. La Base de Datos es la Única Fuente de Verdad.
   - **CIERRE ASÍNCRONO STRICTO:** Todo guardado de cierre de turno debe ejecutarse con `await` para confirmar la transacción en la BD **antes** de ejecutar el `logout` y retornar a login.
   - Toda transacción (Depósito, Retiro, Re-depósito, Recarga, Bóveda) debe afectar de forma atómica y coherente tanto el **INVENTARIO FÍSICO DE PIEZAS** (billetes y monedas en la charola) como los **SALDOS DIGITALES** (`yastasEfectivo`, `yastasTerminal`, `banamex`, etc.).
...
3. **CONTROL DE NAVEGACIÓN, GATEKEEPER Y TURNOS:**
   - **Bloqueo Gatekeeper:** Al autenticarse, si el turno en la BD está `CERRADO`, la interfaz principal debe bloquearse forzando el modal de **Apertura de Turno**. Si el turno anterior sigue `ABIERTO`, debe emitir una alerta bloqueante exigiendo el cierre formal.
   - **Pre-carga y Apertura Matutina:** Evaluar la bandera `precarga_completada` de la BD. Si es `true`, pre-llenar los campos de la apertura; si es `false`, desplegar en $0.00.
   - Permitir la confirmación en 1 solo clic si el físico coincide, o requerir nota de observación si hay discrepancias.

4. **CONTROL OBLIGATORIO DE CACHÉ DE NAVEGADOR:**
...
5. **PREVENCIÓN DE ERRORES EN HTML DINÁMICO (COMILLAS E IDS):**
...
6. **PROTOCOLO DE DIAGNÓSTICO EN NAVEGADOR (PLAYWRIGHT) Y LIMPIEZA DE SIMULACRO:**
...
8. **ESTÁNDAR DE ASISTENTES DE CAMBIO Y OPERACIÓN (AUTO Y MANUAL):**
...
9. **EFICIENCIA DE TOKENS Y MODIFICACIONES INCREMENTALES:**
...
10. **CONSERVACIÓN DE UI, SINCRONIZACIÓN DE RESPALDO Y AUDITORÍA:**
