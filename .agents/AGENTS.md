...
**2026-08-13 (mismo día) — Corrección de "valores fantasma" en campos de monto/texto (`autocomplete="off"`):**
* El usuario reportó que en la computadora del local, al entrar al apartado de Depósito de Yastás, el campo "Monto del Depósito" aparecía con **1210** ya escrito, sin que nadie lo tecleara -- generando una alerta falsa de "Falta por ingresar en charola: $1,210.00".
* Causa raíz: ese valor no existe en ningún lado del código (no es un default ni un placeholder) -- es un valor real que se tecleó alguna vez en ese navegador. Ninguno de los inputs de monto/texto de la app tenía `autocomplete="off"`, así que el navegador (comportamiento nativo de Chrome/Edge de restaurar el último valor tecleado en un campo al recargar la página) lo repone DESPUÉS de que `limpiarDesglose()` ya intentó dejarlo en blanco durante `refrescarPantallas()` -- no es un bug de la lógica de caja ni compromete ningún saldo real.
* Corrección: se agregó `autocomplete="off"` a los 12 campos que `limpiarDesglose()` ya limpia por código (`op-monto-manual`, `op-concepto-transferencia`, `op-motivo-capital`, `op-ubicacion-boveda`, `op-monto-boveda`, `cap-motivo`, `cap-monto-terminal`, `op-cambio-deposito`, `op-retiro-monto`, `op-recarga-monto`, `redep-monto-retiro`, `redep-monto-deposito`) -- mismo riesgo latente en los 12, aunque solo se había manifestado visiblemente en uno.
* Cambio exclusivo de `index.html` (atributo HTML, no toca `app.js`/`app_v3.js` ni ninguna lógica de cálculo/persistencia). Validado: los 12 campos cargan correctamente con `autocomplete="off"` confirmado en el DOM real, sin errores de consola.

**2026-08-14 — Retiro tipo "Getnet" dentro de Yastás, a petición del operador vía el usuario:**
* **Motivación:** el operador pidió separar, dentro del retiro normal de Yastás, los retiros que se cobran por terminal Getnet -- misma mecánica de charola de siempre, pero el valor digital no debe mezclarse con el saldo de Terminal Yastas.
* **Funcionalidad:** nuevo selector "Tipo de Retiro: Normal / Getnet" en el Asistente de Retiros de Yastás (arriba de Sugerido/Manual). La charola (qué billetes salen) es idéntica en ambos casos. La única diferencia es el destino del valor digital:
  - Normal (sin cambios): `yastasEfectivo -= monto`, `yastasTerminal += monto`.
  - Getnet (nuevo): `yastasEfectivo -= monto`, `yastasGetnet += monto` (NO toca Terminal).
* **Prioridad "Normal" siempre por defecto:** el selector se reinicia a Normal dentro de `limpiarDesglose()` (cubre los 5 puntos donde se llama: tras cada transacción exitosa, al cambiar de pestaña/servicio, etc.), no solo en `actualizarFormularioOperacion()` -- decisión explícita del usuario para que nunca quede "pegado" en Getnet.
* **Dónde se ve:** en la misma barra morada "Total Operativo Yastas", reutilizando el slot que ya existía en el HTML para el mismo propósito en T-Conecta (`dash-total-col-retiro` / `dash-total-bar-operator-retiro`) -- no se agregó HTML nuevo a la barra. Ahora se lee: CHAROLA + TERMINAL + GETNET = TOTAL.
* **Reinicio:** `balances.yastasGetnet` se reinicia a 0 en cada cierre de turno (igual que Vales) -- decisión explícita del usuario, distinto al comportamiento persistente de Terminal Yastas.
* **Bitácora:** categoría propia `YASTAS_GETNET` (ya filtrable bajo "Yastás" en el dropdown de cuenta, mismo mecanismo de coincidencia por prefijo que ya usan `YASTAS_RECARGA`/`TCONECTA_RECARGA_*`), con texto amigable "Yastas (Retiro Getnet)" en las vistas de Saldos, Piezas y el modal de desglose.
* **Esquema Supabase:** nueva columna `yastas_getnet numeric not null default 0` en `caja_balances` (migración `supabase/migrations/20260814014249_add_yastas_getnet_column.sql`), conectada en los 3 puntos de sync (upsert, fetchInitial, realtime), igual que `vales`.
* **1 hallazgo corregido durante la implementación:** el early-return de `calcularRetiroOperacion()` (cuando el Asistente de Retiros completo se oculta, ej. al ver Banorte) no ocultaba explícitamente el wrapper del nuevo selector -- en la práctica no se veía (el `display:none` del panel padre ya lo tapaba), pero se corrigió para no depender de esa coincidencia.
* Validado: sintaxis OK, `app.js`/`app_v3.js` idénticos, columna confirmada en Supabase real (`yastas_getnet: 0`), probado en producción real con JavaScript directo (sin completar ninguna transacción real): toggle Normal/Getnet, cálculo correcto de la barra (Charola + Terminal + Getnet = Total, probado en memoria sin `DB.set`), reinicio automático a Normal al cambiar de pestaña/servicio, y ocultamiento correcto del selector fuera de Yastás -- sin errores de consola.

**2026-08-14 (mismo día) — Bug real corregido: "Saldos Operativos" y "Valor Total del Negocio" en el Corte de Caja mezclaban cuentas equivocadas:**
* El usuario reportó (con captura de pantalla de "Corte de Caja de Turno Activo") que las tarjetas 2 y 3 no le cuadraban. Investigación encontró un bug real, no solo de etiqueta: `totalDigital = yastasTerminal + capitalTerminal + tconecta + banamex`, usado tanto en la pantalla en vivo (función ~7695) como en el visor de "Reporte de Cierre Histórico" que reutiliza las mismas tarjetas (función ~8078). La etiqueta decía "Yastas, Meli, T-Conecta" pero el saldo REAL de Meli (`balances.meli`) nunca se incluía -- en su lugar se sumaba el saldo de Sistema Híbrido (`capitalTerminal`), sin mencionarlo en ningún texto. Confirmado con datos reales: Meli tenía $11,598 reales, completamente ausentes de "Saldos Operativos" antes de este fix.
* El mismo patrón exacto (`capitalTerminal` mostrado bajo la etiqueta "Mercado Libre") también existe en la gráfica de dona "Saldos en Terminales" de `renderizarReporteVisual()` (Resumen de Caja guardado, Bitácora) -- **NO se tocó en esta pasada** por decisión explícita del usuario, ya que no fue lo que reportó ni lo que pidió corregir; queda documentado aquí como pendiente conocido para cuando se quiera revisar.
* **Decisión del usuario para simplificar, en vez de "arreglar" la fórmula multi-cuenta:** Tarjeta 2 "Saldos Operativos" pasa a mostrar ÚNICAMENTE `balances.yastasTerminal` (sin Meli, sin Sistema Híbrido, sin T-Conecta, sin Banamex, sin Getnet) -- etiqueta corregida de "Yastas, Meli, T-Conecta" a "Terminal Yastás". Tarjeta 3 "Valor Total del Negocio" pasa a ser Efectivo (charola + bóveda, antes solo charola) + Terminal Yastás -- con esto la etiqueta ya existente "Efectivo + Terminal Yastas" por fin es literalmente cierta, sin necesidad de cambiarla.
...
**2026-09-12 — Descarga automática del Portal Yastás (extensión de navegador) + análisis del encuadre contra la bitácora (Claude Code). Documento completo: `.agents/PLAN_YASTAS_ENCUADRE.md`.**
* **Motivación:** el usuario pidió que desde la app (Vercel), con un botón, se descargue el "Reporte de Movimientos" del Portal de Autoservicio de Yastás para una fecha, y una ventana nueva que lo compare contra los movimientos de la caja del día mostrando quién hizo cada operación, con manejo especial de recargas telefónicas (actualizar saldo + avisar si una recarga no se hizo).
* **Hallazgo que definió la arquitectura:** el portal (Angular 19 + Material, de Gentera) está protegido por el WAF Imperva Incapsula. Se descartó cualquier automatización desde la nube (Vercel/GitHub Actions/VPS: IPs de datacenter) y se descartó explícitamente construir evasión de detección de bots. Decisión con el usuario: **extensión Chrome/Edge (Manifest V3)** que corre en su navegador real; la app solo le manda un mensaje (`externally_connectable`).
* **Qué quedó construido (carpeta nueva `yastas-extension/`, sin tocar `app.js`/`app_v3.js`/`index.html`/Supabase):** `manifest.json`, `background.js` (recibe la orden, abre la pestaña, detecta la descarga con `chrome.downloads`), `content-scripts/watcher.js` (login + secuencia completa grabada por el usuario con `playwright codegen`: menú → Consultas → Reporte de Movimientos → datepicker de Material con navegación de mes → CONSULTAR → EXPORTAR → cerrar sesión), `popup/` (prueba manual por fecha + credenciales en `chrome.storage.local`), `config.local.js` (**en `.gitignore`**, credenciales escritas por el propio usuario), `README.md`.
* **Decisiones explícitas del usuario:** credenciales nunca en código versionado; ritmo "humano" (pausas aleatorias entre pasos, 4–5 s tras entrar) porque "demasiado rápido se ciclaba"; cerrar sesión al final; se usa en Edge.
* **Bugs corregidos durante la sesión (propios de la extensión):** (1) el popup mandaba el mensaje por el canal interno y solo se escuchaba el externo; (2) selector del día del calendario equivocado para Material 19 (`button.mat-calendar-body-cell`, no `-content`) y navegación de mes cuando ya estaba en el mes correcto → oscilaba agosto↔septiembre; (3) el job nunca se marcaba terminado → tras cerrar sesión volvía al login y repetía todo en bucle; ahora hay estados finales (`terminado`/`descarga-completa`/`abortado`) y máximo 2 intentos; (4) el botón "CERRAR SESIÓN" vive en un panel que abre el ícono de usuario arriba a la derecha (capturas del usuario) → detección por posición, no por `img nth(2)`.
* **Validado en el portal real por el usuario:** login → reporte → fecha → CONSULTAR → EXPORTAR (el XLSX baja a Descargas). Cierre de sesión reescrito, pendiente de confirmar en corrida completa.
* **Análisis de datos con el archivo real del 12-sep y `caja_logs` del mismo día:** 20 filas del portal → 11 operaciones de cliente (`OPERACIONES FINANCIERAS` + `CASH-IN/OUT`); el resto son asientos internos (`COBRO DE CUENTA DE FONDEO` en pares espejo, `ABONO A INVERSION`). Resultado: 10 cuadran exacto, 1 con diferencia real (portal `PAGO DE ODP` $8,003.40 vs caja retiro $8,000.00 → $3.40), 0 huérfanos, 0 recargas. Regla clave: comparar contra `Monto Total` (lo que pagó el cliente, comisión incluida), no `Monto Operación`. **Ojo:** en `YASTAS_RECARGA` con método "Pago a Terminal" el `amount` de la bitácora es 0; el monto está en `details`.
* **Entregable visual:** `yastas-extension/docs/encuadre-mockup.html` (+ `.png`): mockup de la ventana con datos reales incrustados y la función `encuadrar()` (reglas de cruce en orden: filtro → tipo → exacto → sin comisión → aproximado → fuera de hora → huérfanos → recargas), portable a `app.js`.
* **Pendiente (plan por fases en el documento):** capturar el contenido del XLSX sin pasar por disco (hook en MAIN world + SheetJS empaquetado), tablas `yastas_movimientos_portal` / `yastas_import_jobs` en Supabase (definir RLS con la key pública), ventana "Encuadre Yastás" en `index.html`/`app.js`/`app_v3.js` con botón "Descargar del portal" y carga manual como respaldo, pruebas Playwright con interceptación de Supabase. **8 preguntas abiertas para el usuario** (qué saldo actualizan las recargas, qué hacer con diferencias, tolerancias, etc.) y **6 puntos donde se pide propuesta de Antigravity** antes de tocar código — ver secciones 8 y 9 del plan.
* **Seguridad:** la contraseña del portal se pegó en el chat durante la sesión; se recomendó rotarla. El agente no la guardó en ningún archivo.

**2026-09-13 — Bloque 1 del Encuadre Yastás integrado en la app (Claude Code, con Ficha de Autorización aprobada por el usuario):**
* **Alcance:** ventana "Encuadre de Caja vs. Yastás" que compara la bitácora del día (`caja_logs`) contra el "Reporte de Movimientos" del portal, con el diseño validado en `yastas-extension/docs/encuadre_*.html` (banner en español llano, 6 tarjetas KPI que son filtros de 1 clic, tabla unificada con avatar del cajero, panel de recargas, movimientos internos, diálogo "¿Cómo calcula el cruce?", modo claro/oscuro local de alto contraste y letra A/A+).
* **`index.html`:** botón `btn-encuadre-yastas` al final de la barra morada "Total Operativo" (L343); `#modal-encuadre-yastas` (z-40, para que `#modal-pin` z-50 quede encima) + `<dialog id="enc-reglas">` + `<style>` propio con tokens `--enc-*`, insertados antes del toast (L2524–2650); SheetJS 0.18.5 desde jsdelivr con `defer` (cdnjs tardaba 10 s y frenaba el arranque); versión `app.js?v=20260912_ENCUADRE`.
* **`app.js` / `app_v3.js` (idénticos, bloque `// === ENCUADRE YASTÁS ===` al final, L9034+):** `abrirModalEncuadreYastas(fecha)`, `cerrarModalEncuadreYastas()`, `cambiarFechaEncuadre()`, `cargarReporteYastasDesdeArchivo(file)` (lee el .xlsx: fila 0 filtros, fila 1 encabezados; la fecha manda desde el archivo y avisa si no coincide con la vista; upsert por `id_operacion` en `yastas_movimientos_portal` + fila en `yastas_import_jobs`; NO persiste Num. Cuenta ni Referencia), `encuadrarYastas(portal, caja, otrosCaja)` (motor: filtro → tipo → exacto → sin comisión → aproximado → error cercano ≤10 min → otra hora → error lejano ≤2 h → dos-en-una → duplicado → otra categoría → huérfanos → ganancia), `encRender*`, `filtrarEncuadre*`, `alternarTemaEncuadre`, `tamanoLetraEncuadre`, `aplicarGananciaTerminal()` → `abrirPINModal` (firma con el PIN del cajero, igual que toda operación) → `encEjecutarAjusteGanancia()`.
* **Reglas de negocio (decisiones explícitas del usuario, ver `PLAN_YASTAS_ENCUADRE.md`):** la terminal se mueve SIEMPRE por `Monto Total` del portal (verificado con Saldo Inicial/Disponible en 58 operaciones); la caja le carga lo que pagó/recibió el cliente; la diferencia (comisión de recargas, vales/ODP) es ganancia → **un** `AJUSTE_DE_SALDO` por día a `balances.yastasTerminal`, **incremental** (`yastas_encuadre_ajustes` guarda el acumulado; una segunda ejecución aplica solo la diferencia; doble clic no duplica). **Prohibido estimar con porcentajes**: solo el valor de la hoja. Recargas se cruzan por `Monto Operación` y vienen como `RECARGA DE TIEMPO AIRE`; `DECLINADA` y consultas de $0 se excluyen. La bitácora no se edita nunca. Errores de captura (dígito faltante/sobrante, ceros/punto, dígitos al revés, sentido invertido, dos-en-una, duplicado, otra cuenta) se DIAGNOSTICAN con corrección sugerida; los botones de corrección quedan para el Bloque 3.
* **Supabase:** migración `supabase/migrations/20260912230000_encuadre_yastas.sql` (tablas `yastas_movimientos_portal`, `yastas_import_jobs`, `yastas_encuadre_ajustes`, RLS `caja_all` como el resto). **Escrita, NO aplicada**: la CLI no tiene sesión (`supabase login` es interactivo). La app avisa con toast si las tablas faltan.
* **Validado (Playwright Chromium headless, `page.route('**/*.supabase.co/**')` bloqueando TODA escritura y simulando las 3 tablas nuevas; lecturas reales de `caja_logs`):** con la bitácora real del 10-sep + el XLSX real de ese día → 47 operaciones, 46 exactas, 1 "otra hora", 0 huérfanos, recargas 3/3, ganancia +$8.75, cajeras Ingrid/Leticia/Mabeli/Miguel (idéntico al mockup); importación de 88 filas con campos correctos y sin sensibles; cambio automático de fecha al día del archivo; filtros, búsqueda, tema, letra, reglas y cierre sin errores JS; ajuste con PIN → Terminal 43,425.00 → 43,433.75, log `AJUSTE_DE_SALDO` firmado por el cajero, upsert del acumulado. `js_validator.py` OK en ambos archivos. Nada commiteado ni subido.
* **Pendiente:** prueba en producción real; Bloque 3 (botones ⚡ de corrección de errores de captura, siempre vía `AJUSTE_DE_SALDO`). `INSTRUCCIONES_CLAUDE_ENCUADRE.md` §"Decisiones de diseño" y el `encuadre_mockup.html` de la raíz quedaron desactualizados frente a estas reglas; la fuente de verdad es `PLAN_YASTAS_ENCUADRE.md` + `yastas-extension/docs/`.

**2026-09-13 (mismo día) — Bloque 2 del Encuadre Yastás: Automatización completa + Subida a Supabase + Borrado Físico de Archivos (Antigravity):**
* **Supabase Realtime:** Se ejecutó `ALTER PUBLICATION supabase_realtime ADD TABLE public.yastas_import_jobs, public.yastas_movimientos_portal, public.yastas_encuadre_ajustes;` en Supabase (`kpddjyytabxuxdjqsjze`). Ahora la base de datos emite eventos reactivos en tiempo real al registrarse nuevos movimientos y jobs del portal.
* **Captura en Memoria (MAIN world):** Creado `yastas-extension/content-scripts/interceptor.js` ejecutándose en el contexto nativo de la página (`"world": "MAIN"`, `document_start`). Intercepta `URL.createObjectURL`, `fetch` y `XMLHttpRequest` para capturar los bytes del archivo `.xlsx` en memoria en cuanto Angular genera la exportación, enviándolo vía `window.postMessage` al content script sin depender del sistema de archivos local.
* **Puente App-Extensión:** Creado `yastas-extension/content-scripts/bridge.js` para enlazar la aplicación web (`caja-registradora-lyart.vercel.app`, localhost) con el Service Worker de la extensión, permitiendo ordenar la descarga con un clic sin necesidad de IDs fijos de extensión.
* **Procesamiento y Supabase en Service Worker (`background.js`):** Integrada librería local `yastas-extension/lib/xlsx.full.min.js`. El Service Worker recibe el archivo en Base64, parsea las celdas (descartando datos sensibles de cuenta y referencia), hace `UPSERT` directo a `yastas_movimientos_portal` y actualiza `yastas_import_jobs` como `'importado'`.
* **Borrado Físico Obligatorio:** En `background.js`, al detectar la descarga (`chrome.downloads.onChanged` con `state === 'complete'`), se ejecuta inmediatamente `chrome.downloads.removeFile(item.id)` para eliminar físicamente el archivo `.xlsx` de la carpeta `Descargas` del disco duro del usuario, y `chrome.downloads.erase({ id: item.id })` para borrar el registro del historial del navegador, evitando acumulación de archivos residuales.
* **Interfaz y Reactividad (`index.html`, `app.js`, `app_v3.js`):**
  - Añadido botón "Descargar del portal" (`#enc-btn-descarga-auto`) en la barra del modal `#modal-encuadre-yastas`, manteniendo la carga manual como respaldo.
  - Implementada función `solicitarDescargaAutomaticaYastas()` y listener de eventos.
  - Añadida suscripción reactiva en `suscribirseARealtimeSupabase()` para la tabla `yastas_import_jobs` que refresca la ventana de encuadre en vivo y muestra confirmación al usuario.
  - Parámetro de versión actualizado a `app.js?v=20260913_ENCUADRE_AUTO`.
* **Validación:** Sintaxis validada con `js_validator.py` (100% válida en ambos archivos). Sin ejecución de `git push` (a la espera de orden explícita del usuario).
