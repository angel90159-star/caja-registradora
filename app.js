// === CATÁLOGOS Y CONFIGURACIÓN ===
    ...
    function completarTransaccion(opName, totalSum) {
      ...
      const inputDep = document.getElementById('op-cambio-deposito');
      const montoDep = inputDep ? parseFloat(inputDep.value) : 0;
      const hasCambio = (isIngreso && !isManualAmount && srv !== 'caja' && montoDep > 0 && totalSum > montoDep && currentSugerenciaCambio);
      ...
      if (srv === 'banorte') {
        ...
        detText = `Depósito de ${fmt.format(finalAmount)}. Recibido: ${fmt.format(totalSum)}. Cambio: ${fmt.format(totalSum - finalAmount)} (${changeTextParts.join(', ')}).`;
      } else if (isRecarga) {
        const recargaMetodo = document.getElementById('op-metodo-recarga-val') ? document.getElementById('op-metodo-recarga-val').value : 'efectivo';
    ...
    function calcularCambioOperacion() {
      const srv = document.getElementById('op-service').value;
      const isIngreso = currentOpType === 'ingreso';
      const isPhysical = (srv === 'yastas' || (srv === 'meli' && document.getElementById('op-modo-meli-val').value === 'tienda'));
      ...
      if (!sessionActive || !isIngreso || !isPhysical) {
        panel.classList.add('hidden');
        currentSugerenciaCambio = null;
        if (panelOrigen) {
          if (isPhysical && sessionActive) {
            panelOrigen.classList.add('hidden'); // Siempre oculto para estos servicios
          } else if (sessionActive && srv === 'caja') {
            panelOrigen.classList.remove('hidden'); // Mostrar para Bóveda que sí usa desglose de charola directo
          } else {
            panelOrigen.classList.add('hidden');
          }
        }
      ...
      if (panelOrigen) panelOrigen.classList.add('hidden'); // Ocultar siempre el duplicado para yastas/meli/tconecta
      ...
      // Leer Monto del Depósito
      const inputDep = document.getElementById('op-cambio-deposito');
      const montoDep = parseFloat(inputDep.value) || 0;
      ...
      if (montoDep > 0) {
        ...
        // CASO C: Se requiere dar cambio al cliente
          ...
          // Ordenar midDenoms y coinDenoms por cantidad de stock descendente
          // para dar preferencia a vaciar las denominaciones más saturadas
          midDenoms.sort((a, b) => (inventory[b] || 0) - (inventory[a] || 0));
          coinDenoms.sort((a, b) => (inventory[b] || 0) - (inventory[a] || 0));

          // Lista de prioridad de búsqueda para el cambio:
          // Primero intentamos con los billetes medianos/bajos (ordenados por volumen).
          // Si queda residuo, usamos las monedas (ordenadas por volumen).
          // Si aun así no podemos completar, al final permitimos usar billetes altos (1000 y 500).
          const searchList = [
            ...midDenoms,
            ...coinDenoms,
            ...highDenoms.sort((a, b) => b - a)
          ];
